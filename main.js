...
function initiateAttack() {
    ...
    if (oppMonsters.length > 0) { oppMonsters.forEach(m => { m.parentElement.classList.add('targetable'); }); } 
...
function performAction(action) {
    ...
    const cardData = {
        ...
        level: selectedHandCard.getAttribute('data-level'),
        attribute: selectedHandCard.getAttribute('data-attribute'),
        race: selectedHandCard.getAttribute('data-race')
    };
    ...
    selectedHandCard.remove(); selectedHandCard = null;
    actionMenu.classList.remove('active');
